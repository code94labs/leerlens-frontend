...
const DynamicDropdown = ({ options, setOptions }: Props) => {
  ...
  const handleShowAddNewOption = () => setDisplayAddNewOption(true);

  const handleAddNewOptionDelete = () => {
    formik.resetForm();
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      setDisplayAddNewOption(false);
      handleAddNewOption(values.value);
      resetForm();
    },
  });
    ...
    <FormControl sx={{ width: "50%" }}>
      ...
      >
        <ListSubheader disableSticky>
          ...
          >
            <Typography sx={customStyles.buttonText}>Add new option</Typography>
          </Button>
        </ListSubheader>
        {displayAddNewOption && (
          <ListSubheader disableSticky>
              ...
              {item.isNewlyAdded && (
                <Stack direction="row" alignItems="center" gap={1}>
                  {editableOptions.includes(item.id) ? (
                    ...
                    >
                      <Typography sx={customStyles.buttonText}>save</Typography>
                    </Button>
                  ) : (
                    <IconButton
                      aria-label="edit"
